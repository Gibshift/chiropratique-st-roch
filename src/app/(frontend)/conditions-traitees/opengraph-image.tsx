...
export const alt = 'Conditions traitées — Chiropratique St-Roch'
...
export default async function Image() {
  const [watercolorData, imgData] = await Promise.all([
    ...
    readFile(path.join(process.cwd(), 'public/assets/femme-questionnement-siteweb-droite.png')),
  ])
  ...
  return new ImageResponse(
    (
      <div style={{ width: '1200px', height: '630px', display: 'flex', backgroundColor: '#f6f1e8', position: 'relative' }}>
        ...
        {/* Illustration droite */}
        <img src={imgSrc} style={{ position: 'absolute', right: 0, top: 0, width: '480px', height: '100%', objectFit: 'contain', objectPosition: 'right bottom', opacity: 0.9 }} />


        {/* Texte gauche */}
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 80px', width: '720px', position: 'relative', zIndex: 1 }}>
          <span style={{ fontSize: 13, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.16em', color: '#dc2626' }}>
            Conditions traitées
          </span>
          <div style={{ display: 'flex', flexDirection: 'column', marginTop: 16 }}>
            <span style={{ fontSize: 88, fontWeight: 700, textTransform: 'uppercase', lineHeight: 1, color: '#18181b' }}>Vous avez</span>
            <span style={{ fontSize: 88, fontWeight: 700, textTransform: 'uppercase', lineHeight: 1, color: '#18181b' }}>mal où?</span>
          </div>
